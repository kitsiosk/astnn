...
class Pipeline:
    ...
    def read_pairs(self, filename: str):
        ...
        """
        pairs = pd.read_pickle(os.path.join(self.root, self.language, 'scb', filename))
    ...
    def dictionary_and_embedding(self, input_file, size):
        ...
        if not os.path.exists(data_path+'scb/embedding'):
            os.mkdir(data_path+'scb/embedding')
        if self.language == 'c':
            sys.path.append('../')
        ...
        w2v.save(data_path+'scb/embedding/node_w2v_' + str(size))
    ...
    def generate_block_seqs(self):
        ...
        word2vec = Word2Vec.load(
            self.root + '/'+ self.language+'/scb/embedding/node_w2v_' +
            str(self.size)
        ).wv
    ...
    def run(self):
        ...
        # )
        input_file = (
            'programs.pkl' if self.language == 'c' else 'scb/scb_funcs_all.pickle'
        )

        if os.path.exists(os.path.join(self.root, self.language, 'scb', 'ast.pkl')):
            print('a')
            self.get_parsed_source(input_file='scb/ast.pkl')
        else:
            self.get_parsed_source(input_file=input_file,
                                   output_file='scb/ast.pkl')
        print('read id pairs...')
        if self.language == 'c':
            self.read_pairs('oj_clone_ids.pkl')
        else:
            #self.read_pairs('bcb_pair_ids.pkl')
            self.read_pairs('scb_pair_ids.pickle')

        # print('split data...')
        # self.split_data()
        path_all = self.root+'/'+self.language+'/scb/'
        ...
        self.merge(path_all + 'pairs.pickle', path_all + 'blocks.pickle')
